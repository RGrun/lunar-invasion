...
import r3software.org.lunarinvasion.levels.L_Adventure;
 ...
 *
 * First level select screen
 */
public class LevelSelect1 extends GLScreen {
        ...
        for(int i = 0; i < len; i++) {
            ...
            if(event.type == Input.TouchEvent.TOUCH_UP) {
                ...
                if(OverlapTester.pointInRectangle(level1, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_Adventure()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level2, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_Arena()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level3, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_AroundTheCorner()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level4, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_Backboard()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level5, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_Cage()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level6, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_CannonFodder()));
                    return;
                }
    ...
    public void present(float deltaTime) {
        ...
        batcher.beginBatch(Assets.select1);
        batcher.drawSprite(360, 640, 720, 1280, Assets.select1Region);
        batcher.endBatch();
