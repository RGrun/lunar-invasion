 ...
 *
 * Third level select screen
 */
public class LevelSelect3 extends GLScreen {
        ...
        for(int i = 0; i < len; i++) {
            ...
            if(event.type == Input.TouchEvent.TOUCH_UP) {
                ...
                if(OverlapTester.pointInRectangle(level1, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_Highway()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level2, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_Honeypot()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level3, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_Hourglass()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level4, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_Isolation()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level5, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_LunarBurger()));
                    return;
                }

                if(OverlapTester.pointInRectangle(level6, touchPoint)) {
                    Assets.playSound(Assets.menuClick);
                    game.setScreen(new GameScreen(game, new L_LunarCheeseburger()));
                    return;
                }
    ...
    public void present(float deltaTime) {
        ...
        batcher.beginBatch(Assets.select3);
        batcher.drawSprite(360, 640, 720, 1280, Assets.select3Region);
        batcher.endBatch();
